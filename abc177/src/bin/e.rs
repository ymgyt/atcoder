...
fn main() {
    ...
    let factorizer = PrimeFactorizer::prepare(1000_000);
    ...
    let mut is_pairwise = || {
        ...
        for &i in &a {
            buf.clear();
            factorizer.factorize(i, &mut buf);
            buf.sort_unstable();
            buf.dedup();
            for &factor in &buf {
                if let Some(_) = factors.insert(factor) {
                    return false;
                }
            }
        }
    ...
    let init = a[0];
    let is_setwise = a.into_iter().skip(1).fold(init, |acc, n| gcd(acc, n)) == 1;
