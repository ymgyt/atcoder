...
fn main() {
    ...
    let a = scanner.collect::<usize>(n - 1);
    let mut v = vec![Vec::new(); n];

    a.into_iter().enumerate().for_each(|(i, boss)| {
        let employee_num = i + 1;
        v[boss - 1].push(employee_num);
    });

    v.into_iter()
        .for_each(|members| println!("{}", members.len()));
}
