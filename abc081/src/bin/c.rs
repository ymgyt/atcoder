...
fn main() {
    ...
    let (n, k) = scanner.tuple_2::<usize, i64>();
    let v = scanner.collect::<i64>(n);

    let h = v.into_iter().fold(HashMap::new(), |mut h, n| {
    ...
    let answer = {
        if h.len() <= k as usize {
            ...
            let mut v: Vec<(i64, i32)> = h.into_iter().collect();
            v.sort_unstable_by(|(_, n1), (_, n2)| n1.cmp(n2));
            let l = v.len();
            v.into_iter().take(l - k as usize).fold(0, |acc, (_, n)| {
                acc + n
            })
        }
    };
