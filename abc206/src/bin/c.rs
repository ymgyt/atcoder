...
fn main() {
    ...
    let (_, answer) = v.into_iter().enumerate().fold((HashMap::new(), 0), |(mut h, mut answer), (idx, n)| {
        answer += (idx as i64) - h.get(&n).unwrap_or(&0_i64);
        h.entry(n).and_modify(|n| *n = *n + 1).or_insert(1_i64);
        (h, answer)
    });
