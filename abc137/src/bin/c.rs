...
fn main() {
    ...
    let n = scanner.scan();
    let mut v: Vec<String> = scanner.collect(n);
    let mut h: HashMap<String, i64> = HashMap::new();

    v.into_iter().for_each(|s|  {
        let sorted = s.chars().sorted().collect::<String>();
        h.entry(sorted).and_modify(|n| *n = *n + 1).or_insert(1);
    });

    let answer = h.into_iter().fold(0 ,|acc, (_,n)| {
        acc + n * (n-1) /2
    }  );
