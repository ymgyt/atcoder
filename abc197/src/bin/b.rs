...
fn main() {
    ...
    let (h,w): (usize, usize) = scanner.tuple_2();
    let (y,x): (usize, usize) = scanner.tuple_2();
    let mut vec: Vec<Vec<char>> = Vec::with_capacity(h);

    for _ in 0..h {
        let s = scanner.scan::<String>();
        vec.push(s.chars().into_iter().collect());
    }

    // convert 0 origin
    let (x,y) = (x-1, y-1);
    let mut count = match vec[y][x] {
       '.' => 1,
        _ => {
            println!("0");
            return
