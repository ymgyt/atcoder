...
fn main() {
    ...
    let (h, w, k) = scanner.tuple_3::<usize, usize, usize>();
    let mut c = Vec::with_capacity(h);
    for _ in 0..h {
        let s = scanner.scan::<String>();
        c.push(s.chars().collect::<Vec<char>>());
    }

    let mut count = 0;
    for row_bits in 0..(1 << h) {
        for col_bits in 0..(1 << w) {
            let mut curr = 0;
            ...
            for row in 0..h {
                ...
                for col in 0..w {
                    ...
                    curr += if c[row][col] == '#' { 1 } else { 0 };
                }
            }

            count += if curr == k { 1 } else { 0 };
        }
    }
