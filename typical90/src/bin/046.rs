...
fn main() {
    ...
    let a = scanner.collect::<i64>(n);
    let b = scanner.collect::<i64>(n);
    let c = scanner.collect::<i64>(n);

    let (a, b, c) = a
        .into_iter()
        .map(|n| n % 46)
        .zip(b.into_iter().map(|n| n % 46))
        .zip(c.into_iter().map(|n| n % 46))
        .fold(
            (HashMap::new(), HashMap::new(), HashMap::new()),
            |(mut a, mut b, mut c), ((n_a, n_b), n_c)| {
                a.entry(n_a).and_modify(|c| *c = *c + 1).or_insert(1_i64);
                b.entry(n_b).and_modify(|c| *c = *c + 1).or_insert(1_i64);
                c.entry(n_c).and_modify(|c| *c = *c + 1).or_insert(1_i64);
                (a, b, c)
            },
        );

    let mut answer = 0;
    for (a_n, a_c) in a.iter() {
        for (b_n, b_c) in b.iter() {
            for (c_n, c_c) in c.iter() {
                if (*a_n + *b_n + *c_n) % 46 == 0 {
                    answer += *a_c * *b_c * *c_c;
                }
            }
