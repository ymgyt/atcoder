...
fn main() {
    ...
    let (_, ans) = s
        ...
        .cycle()
        .take(s.len() * k)
        .fold((None, 0), |(cur, ans), c| match cur {
            None => (Some(Current::Char(c)), ans),
            Some(Current::Char(c2)) => {
                if c == c2 {
                    (Some(Current::Operated), ans + 1)
                } else {
                    (Some(Current::Char(c)), ans)
                }
            }
            Some(Current::Operated) => (Some(Current::Char(c)), ans),
        });
