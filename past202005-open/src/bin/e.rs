...
impl Graph {
    fn invoke(&mut self, at: usize) {
        let color = self.colors[at];
        let adj = &self.nodes[at].clone();
        adj.iter().cloned().for_each(|i| {
            self.colors[i as usize] = color;
        })
    }

    fn change_color(&mut self, at: usize, to: usize) {
        self.colors[at] = to;
    }

    fn color(&self, at: usize) -> usize {
        self.colors[at]
    }
}
...
fn main() {
    ...
    let (n, m, q) = scanner.tuple_3::<usize, usize, usize>();
    let mut nodes: Vec<Vec<usize>> = Vec::with_capacity(n);
    for i in 0..n {
        nodes.push(Vec::new());
    }
    for _ in 0..m {
        let (u, v) = scanner.tuple_2::<usize, usize>();
        nodes[u - 1].push(v - 1);
        nodes[v - 1].push(u - 1);
    }
    let c: Vec<usize> = scanner.collect(n as usize);

    let mut g = Graph { nodes, colors: c };

    for _ in 0..q {
        let query = scanner.scan::<i64>();
        match query {
            1 => {
                let x = scanner.scan::<usize>();
                let x = x - 1;
                println!("{}", g.color(x));
                g.invoke(x);
            }
