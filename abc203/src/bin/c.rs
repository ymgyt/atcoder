...
fn main() {
    ...
    let (n, mut k) = scanner.tuple_2::<i64, i64>();
    let mut vec = Vec::with_capacity(n as usize);
    (0..n).into_iter().for_each(|_| vec.push(scanner.tuple_2::<i64, i64>()));

    vec.sort_unstable_by_key(|t| std::cmp::Reverse(t.0));

    let mut i = 0;
    while k > 0 {
        i += k;
        k = 0;
        while let Some((a, b)) = vec.pop() {
            if i >= a {
                k += b;
            } else {
                vec.push((a,b));
