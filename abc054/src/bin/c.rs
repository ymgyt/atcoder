...
fn main() {
    ...
    let mut ans = 0;
    let search = Search { nodes: Vec::new() };
    let mut queue = std::collections::VecDeque::new();
    queue.push_back((search, 0));

    while let Some((mut search, node)) = queue.pop_front() {
        search.nodes.push(node);

        if search.nodes.len() == n {
            ans += 1;
            continue;
        }
        ...
        g[node]
            .iter()
            .filter(|adj| !search.nodes.contains(adj))
            .for_each(|adj| {
                queue.push_front((search.clone(), *adj));
            });
    }
