...
fn solve(nums: &Vec<i64>, ops: &mut Vec<char>) {
    if ops.len() == 3 {
        let first = nums[0];

        let answer = (&nums[1..]).iter().zip(ops.iter()).map(|(n, c)| {
            match c {
                '+' => *n,
                '-' => -1 * (*n),
                _ => unreachable!()
...
fn main() {
    ...
    let v: Vec<i64> = scanner.scan::<String>().chars().map(|c| c.to_digit(10).unwrap() as i64).collect();
    let mut ops = Vec::new();
