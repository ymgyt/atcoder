...
fn g(x: impl Into<String>,descending: bool ) -> u64 {
    let mut vec = x.into().chars().into_iter()
        .map(|c| u64::from_str(c.to_string().as_str()).unwrap())
        .collect::<Vec<u64>>();
    vec.sort();
    if descending {
        vec.reverse();
    ...
    vec.into_iter().enumerate()
        .fold(0, |acc, (idx, n)| {
                n * 10_u64.pow((len - 1 - idx) as u32) + acc
        })
}

fn g1(x: impl Into<String>) -> u64 {
    g(x, true)
}

fn g2(x: impl Into<String>) -> u64 {
    g(x, false)
}

fn x(x: impl Into<String>) -> u64 {
    let x = x.into();
    g1(&x) - g2(x)
}
...
fn main() {
    ...
    let (mut n, k): (u64, usize) = scanner.tuple_2();
...
mod tests {
   use super::*;
    ...
    #[test]
    fn test() {
        assert_eq!(g1("0123"), 3210);
        assert_eq!(g2("0123"), 123);
    }
}
