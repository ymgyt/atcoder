...
fn main() {
    ...
    let _n: i64 = scanner.scan();
    let s: String = scanner.scan();

    let mut answer: usize = 0;
    let mut a: HashMap<char, i64> = HashMap::new();
    let mut b: HashMap<char, i64> = HashMap::new();

    for c in s.chars() {
        b.entry(c).and_modify(|n| *n = *n + 1).or_insert(1);
    }

    for c in s.chars() {
        a.entry(c).and_modify(|n| *n = *n + 1).or_insert(1);
        ...
        let hit = a.keys().fold(0, |hit, a_key| {
