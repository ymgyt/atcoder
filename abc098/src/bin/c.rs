...
fn main() {
    ...
    let s: ascii::AsciiString = scanner.scan();

    let mut sum_w = vec![0; n + 1];
    let mut sum_e = vec![0; n + 1];

    for i in 1..=n {
        sum_w[i] = sum_w[i - 1] + if s[i - 1] == AsciiChar::W { 1 } else { 0 };
    }
    for i in (0..n).rev() {
        sum_e[i] = sum_e[i + 1] + if s[i] == AsciiChar::E { 1 } else { 0 };
    }

    let mut ans = std::usize::MAX;
    for i in 0..n {
        ans = cmp::min(ans, sum_w[i] + sum_e[i + 1]);
    }
